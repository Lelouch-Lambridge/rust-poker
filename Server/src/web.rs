...
<head>
  ...
  <style>
    ...
    .pot-center { display: grid; justify-items: center; gap: 6px; min-width: 150px; padding: 10px 16px; border: 1px solid rgba(240,211,109,.42); border-radius: 8px; background: rgba(8,28,25,.78); box-shadow: 0 12px 28px rgba(0,0,0,.24); }
    ...
    .chip-row { display: flex; align-items: center; gap: 8px; margin-top: 8px; min-height: 30px; }
    .chip-stack { position: relative; width: 48px; height: 28px; flex: 0 0 auto; }
    .chip { position: absolute; width: 28px; height: 28px; border-radius: 50%; border: 3px dashed rgba(255,255,255,.78); box-shadow: inset 0 0 0 4px rgba(0,0,0,.16), 0 4px 8px rgba(0,0,0,.28); }
    .chip:nth-child(1) { left: 0; z-index: 4; }
    .chip:nth-child(2) { left: 7px; z-index: 3; }
    .chip:nth-child(3) { left: 14px; z-index: 2; }
    .chip:nth-child(4) { left: 21px; z-index: 1; }
    .chip.red-chip { background: #b93c3c; }
    .chip.gold-chip { background: #d9b64c; }
    .chip.blue-chip { background: #336fa3; }
    .chip.green-chip { background: #2f8c62; }
    .chip-label { color: #d3dacd; font-size: 12px; font-weight: 800; }
...
<body>
    ...
    <section class="table">
      ...
      <div class="felt">
        ...
        <div class="board-panel">
          <div class="pot-center"><span>Pot</span><strong id="pot">0</strong></div>
          <h2>Table Cards</h2>
  ...
  <script>
    ...
    function chipColors(amount) {
      const count = Math.max(1, Math.min(4, Math.ceil(Number(amount || 0) / 250)));
      const colors = ["red-chip", "gold-chip", "blue-chip", "green-chip"];
      return colors.slice(0, count);
    }

    function renderChipStack(target, amount) {
      target.innerHTML = `<div class="chip-stack">
        ${chipColors(amount).map(color => `<span class="chip ${color}"></span>`).join("")}
      </div>
      <span class="chip-label">${amount ?? 0}</span>`;
    }
    ...
    function render(state) {
      ...
      (state.players || []).forEach(player => {
        ...
        renderChipStack(el.querySelector(".wallet-chips"), player.wallet);
        el.querySelector(".wallet-chips .chip-label").prepend("Wallet: ");
        renderChipStack(el.querySelector(".bet-chips"), player.bet);
        el.querySelector(".bet-chips .chip-label").prepend("Bet: ");
